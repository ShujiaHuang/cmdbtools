...
def login(token, url):
    ...
    test_url = "{}/api/v1.0/variant?token={}&type=position&query=chr17-41234470".format(url, token)
...
def query_variant(chromosome, position):
    ...
    query_url = '{}/variant?&type=position&query={}-{}'.format(tokenstore["url"], chromosome, position)
